    ...
    if (isset($_FILES["photo"]) && $_FILES["photo"]["error"] <= 0) {
        ...
        if (in_array($filetype, $allowed)) {
            ...
            // else{
                ...
                $move = move_uploaded_file($_FILES["photo"]["tmp_name"], "user/". $filename);

                if ($move) {
                    
                    $insert = "INSERT INTO `users` (`id`,`username`,`password`,`picture`,`date_created`) VALUES (NULL,'".$username."','".md5($password)."','".$filename."',CURRENT_TIMESTAMP())";
